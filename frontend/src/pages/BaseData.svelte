<script>
  ...
  async function createBase() {
    if (gender === "") {
      ...
      if (response.ok) {
        ...
        alert(base_data["firstname"] + " wurde in der Datenbank gespeichert.");
        push("/semester-data");
      } else {
  ...
  function cancel() {
    if (confirm("Möchtest Du das Formular leeren?")) {
      firstname =
        lastname =
        gender =
        birthday =
      ...
      <tr>
        ...
        <td>
          <input
            type="text"
            class="form-control"
            bind:value={special_education_teacher}
            placeholder="Heilpaedagogin"
            required
          />
        </td>
      </tr>
